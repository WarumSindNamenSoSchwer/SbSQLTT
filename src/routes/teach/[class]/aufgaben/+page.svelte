...
<div class="max-w-[1280px] mx-auto px-6 py-8 md:py-10 pb-24">
		...
		<div class="flex items-center gap-2 flex-wrap">
			<div class="relative">
				...
				<input
					...
					class="h-9 w-56 pl-9 pr-3 rounded-md bg-ink-50 border border-ink-200 text-[13px] placeholder-ink-600 ring-accent focus:border-[oklch(0.72_0.15_195_/_0.5)]"
				/>
			</div>
			<Button variant="ghost" size="md" onclick={enableAll}
	...
	<div class="grid grid-cols-1 md:grid-cols-3 gap-5">
		{#each AUFGABEN_TRACKS as t, ti (t.id)}
			{@const filteredLessons = t.lessons.filter(
				(l) =>
			...
			<Surface class="p-0 flex flex-col">
				<div
					class="px-4 py-3 border-b border-ink-200 flex items-center justify-between bg-ink-50/40"
				>
				...
				<ul class="p-2 flex flex-col gap-1.5">
					{#each filteredLessons as l (l.id)}
						...
						>
							<span
								class={'shrink-0 inline-flex items-center justify-center w-6 h-7 rounded text-ink-600 ' +
									(on
							...
							<button
								...
								class={'shrink-0 inline-flex items-center justify-between w-[88px] h-7 pl-2 pr-1 rounded-full text-[11px] font-mono transition-colors ' +
									(on
